...
chatpers_vector_db_dir=f"{parent_dir}/chapters_vector_db"
...
def vectorize_chapters(class_subject_name):
    ...
    for chapter in os.listdir(book_dir):
        ...
        Chroma.from_documents(documents=texts,embedding=embedding,persist_directory=f"{chatpers_vector_db_dir}/{chapter_name}")
        print(f"{chapter_name} chapter vectorised")
    return 0
